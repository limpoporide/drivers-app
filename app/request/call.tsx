...
export default function RequestCallScreen() {
  ...
  const resolvedParticipantName = getFirstName(normalizeRouteParam(participantName).trim() || 'Rider');
  ...
  useEffect(() => {
    console.warn(
      `[Agora Driver Call] Screen mounted: ${JSON.stringify({
        bookingId: resolvedBookingId,
        participantName: resolvedParticipantName,
        callUUID: resolvedCallUUID || null,
  ...
  const notifyIncomingCall = async () => {
    if (!resolvedBookingId || ringSignalSentRef.current || isIncomingCall || hasExistingOutgoingSignal) {
      console.warn(
        `[Agora Driver Call] Skipping incoming-call broadcast: ${JSON.stringify({
          bookingId: resolvedBookingId,
          ringSignalSent: ringSignalSentRef.current,
          incoming: isIncomingCall,
          hasExistingOutgoingSignal,
        })}`
      );
      return;
    }
    ...
    try {
      ...
      console.warn(
        `[Agora Driver Call] Subscribing to broadcast channel: ${JSON.stringify({
          channel: `ride_call:${resolvedBookingId}`,
        })}`
      );
      ...
      const isChannelReady = await waitForBroadcastChannel(
        ...
        (status) => {
          console.warn(
            `[Agora Driver Call] Broadcast channel status: ${JSON.stringify({
              bookingId: resolvedBookingId,
              status,
            })}`
          );
      ...
      console.log('[Agora Driver Call] Broadcasting incoming call', {
        bookingId: resolvedBookingId,
        participantName: resolvedParticipantName,
      });
      ...
      console.warn(
        `[Agora Driver Call] Incoming-call broadcast sent: ${JSON.stringify({
          bookingId: resolvedBookingId,
          participantName: resolvedParticipantName,
        })}`
      );
      ...
      console.log('[Agora Driver Call] Failed to broadcast incoming call', error);
    }
  };
  ...
  const cleanupCall = () => {
    ...
    try {
      ...
      console.log('[Agora Driver Call] Cleanup error', error);
    } finally {
  ...
  const emitCallSignal = async (event: 'call_ended') => {
    ...
    try {
      ...
      console.log('[Agora Driver Call] Failed to send call signal', { event, error });
    } finally {
      supabase.removeChannel(channel);
    }
  };
  ...
  const requestMicrophonePermission = async () => {
    ...
    const result = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.RECORD_AUDIO,
      {
        ...
        message: 'Limpopo Driver needs microphone access so you can talk to the rider in the app.',
  ...
  useEffect(() => {
    ...
    const bootstrapCall = async () => {
      console.warn(
        `[Agora Driver Call] Bootstrap start: ${JSON.stringify({
          bookingId: resolvedBookingId,
          participantName: resolvedParticipantName,
          callUUID: resolvedCallUUID || null,
          incoming: isIncomingCall,
      ...
      if (!resolvedBookingId) {
        ...
        setCallError('This request is missing the call details needed to start an in-app call.');
        return;
      }
      ...
      console.warn(
        `[Agora Driver Call] Microphone permission result: ${JSON.stringify({
      ...
      try {
        ...
        console.warn(
          `[Agora Driver Call] Requesting Agora token: ${JSON.stringify({
            bookingId: resolvedBookingId,
          })}`
        );
        ...
        console.warn(
          `[Agora Driver Call] Received token payload: ${JSON.stringify({
            bookingId: resolvedBookingId,
            channelName: callCredentials?.channelName ?? null,
            uid: callCredentials?.uid ?? null,
            hasAppId: Boolean(callCredentials?.appId),
            hasToken: Boolean(callCredentials?.token),
          })}`
        );
        ...
        const eventHandler: IRtcEngineEventHandler = {
          ...
          onError: (err, msg) => {
            console.log('[Agora Driver Call] Engine error', { err, msg });
        ...
        console.warn(
          `[Agora Driver Call] joinChannel result: ${JSON.stringify({
            bookingId: resolvedBookingId,
            joinResult,
          })}`
        );
        ...
        console.log('[Agora Driver Call] Bootstrap failed', error);
  ...
  }, [resolvedBookingId, resolvedCallUUID, resolvedParticipantName]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('broadcast', { event: 'call_ended' }, ({ payload }) => {
        ...
        console.log('[Agora Driver Call] Remote call ended', {
